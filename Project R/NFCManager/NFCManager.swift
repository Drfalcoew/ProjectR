...
open class NFCManager: NSObject {
    ...
    /// - Parameter didDetect: Gets called when the manager detects NFC tag or occurs some errors
    open func write(message: String, didBecomeActive: DidBecomeActive? = nil, didDetect: @escaping DidDetect) throws {
        guard NFCNDEFReaderSession.readingAvailable else {
            self.didDetect?(self, .failure(.unavailable))
            return
        }
        ...
        func createLink(name: String) throws -> NFCNDEFMessage? {
            // 1
            guard let payload = NFCNDEFPayload.wellKnownTypeURIPayload(url: URL(fileReferenceLiteralResourceName: message))
              else {
                throw NFCError.invalidPayloadSize
            }
        ...
        action = try .write(message: createLink(name: message)!)
